...
needIndustry = ['M1722 生技醫療','M1800 玻璃陶瓷','M1900 造紙工業']
'''
1. 從總經理、財務長...等等，挑選符合的欄位。若有大於1人以上，則挑選職位最高者。若還是大於1人，則利用關鍵字篩選。
2. 若某職位中，無人符合，則將該職位的符合人數、不符合人數、無法辨識學院人數都設為0
3. 建立"專業度"欄位，若無法辨識，則為"無法辨識學院"，若符合，則為"是"，反之為"否"。但若為無法辨識且高中職以下，則設為"否"
4. 計算每間公司、每年，符合人數、不符合人數、無法辨識學院人數
5. 計算分數：只要有1人以上符合，就算該職位分數為1，其餘為0
6. 建立"高階經理人_總分"欄位，為全部職位分數加總
'''
...
# 修改 20201201
industry_department = pd.read_excel('./data/0_科系_學院分類.xlsx')
industry_department_exception = pd.read_excel('./data/0_科系_學院分類_例外.xlsx')
industry_department['系所'] = industry_department['系所'].apply(lambda x:x.replace('(','\\(').replace(')','\\)'))
industry_department_exception['系所'] = industry_department_exception['系所'].apply(lambda x:x.replace('(','\\(').replace(')','\\)'))
...
#     global industryData,managerDict,position
    ...
    def getManagerDict():  # 各產業高階經理人的關鍵字、專業學院
        ...
        # 修改 20201201
        global manage_department
        global law_department
        global it_department
        global ciber_secure_department
        global industryProdepartment
        global industryProdepartment_exception
        managerDict = {'總經理':
                           {'包含':'總經理|執行長|總裁',
                            ...
                            '專業科系':f'{manage_department}|{industryProdepartment_exception}|{industryProdepartment}'.replace('||','|')
                            },
                       '財務長':
                           {'身份別':'財務主管|財會主管',
                            '包含':'財務長',
                            ...
                            '專業學院':'管理學院',
                            '專業科系':f'{manage_department}'.replace('||','|')
                            },
                       '營運長':
                           {'職稱':'營運',
                            '包含':'營運長',
                       ...
                       '法遵長':
                           ...
                           '專業科系':f'{law_department}|{industryProdepartment_exception}|{industryProdepartment}'.replace('||','|')
                           },
                       '法務長':
                           {'職稱':'法務|法律事務',
                       ...
                       '資安長':
                            {'職稱':'資安|資訊安全',
                             ...
                             '專業學院':'電機資訊學院',
                             '專業科系':f'{ciber_secure_department}'.replace('||','|')
                             }
    ...
    def getAndMergeInfo(df):
        ...
        if len(df.index) == 0:
            ...
            # 修改 20201201
            managerTable[position+'_科系'] = np.nan
            ...
            # 修改 20201201
            managerNameId = df.groupby(['上市別','公司代碼簡稱','資料源年月']).agg(
                                                                                    姓名代碼 = ('姓名代碼',lambda x: '&'.join(x)),
                                                                                    董監經理人姓名 = ('董監經理人姓名',lambda x: '&'.join(x)),
                                                                                    職位代碼_排序 = ('職位代碼_排序', lambda x : min(x)),
                                                                                    教育程度 = ('教育程度', lambda x : '&'.join(x)),
                                                                                    年資經理人 = ('年資--經理人', lambda x :'&'.join(x)),
                                                                                    職稱 = ('職稱' , lambda x : '&'.join(x)),
                                                                                    身份別 = ('身份別', lambda x : '&'.join(x)),
                                                                                    學經歷及目前兼任說明 = ('學經歷及目前兼任說明' , lambda x : '#'.join(x)),
                                                                                    學院 = ('學院', lambda x : '#'.join(x)),
                                                                                    科系 = ('科系', lambda x : '#'.join(x)),
                                                                                    期初股數 = ('期初股數', lambda x : sum(x)),
                                                                                    期末股數 = ('期末股數', lambda x : sum(x)),
                                                                                    期初股數合計 = ('期初股數(合計)', lambda x : sum(x)),
                                                                                    期末股數合計 = ('期末股數(合計)', lambda x : sum(x))
                                                                                    ).reset_index()
            managerNameId.rename(columns = { '年資經理人' : '年資--經理人',
                                            '期初股數合計': '期初股數(合計)',
                                            '期末股數合計': '期末股數(合計)'
                                            }, inplace = True)
    ...
    def countAndMergeNumPro(df):
        ...
        if len(df.index) == 0:  #如果此類高階經理人，無資料
            ...
            # 修改 20201201
            managerTable[position+'_無法辨識科系人數'] = 0
            return managerTable       
            ...
            # 修改 20201201
            positionNeedDepartment = managerDict[position]['專業科系']
            df['專業度'] = np.where(df['學院'] == '無法辨識', '無法辨識科系',
                                np.where(df['科系'].str.contains(positionNeedDepartment), '是', '否'))
            # if == '無法辨識'
            #    return '無法辨識'
            # else:
            #    if 符合專業標準:
    ...
    def getLabelAndScore():
        ...
        managerTable[position+'_燈號'] = np.where(managerTable[position+'_符合人數'] > 0, '符合',
                                           np.where(managerTable[position+'_不符合人數'] > 0, '不符合',
                                               ...
                                               np.where(managerTable[position+'_無法辨識科系人數'] > 0, '無法辨識科系',
                                                   '沒有揭露')))
...
for industry in needIndustry:
    ...
    # 修改 20201201
    # 可針對不同管理階層，建立不同的manage_department 清單
    manage_department = ('|'.join(industry_department.query('院=="管理學院"')['系所'].to_list()) + '|' + \
                            '|'.join(industry_department_exception.query('院=="管理學院"')['系所'].to_list())).replace('||','|')
                            
    law_department = ('|'.join(industry_department.query('院=="法學院"')['系所'].to_list()) + \
                        '|'.join(industry_department_exception.query('院=="法學院"')['系所'].to_list())).replace('||','|')
                        
    it_department = ('|'.join(industry_department.query('院=="電機資訊學院" or 院=="工學院"')['系所'].to_list()) + \
                        '|'.join(industry_department_exception.query('院=="電機資訊學院" or 院=="工學院"')['系所'].to_list())).replace('||','|')
                        
    ciber_secure_department = ('|'.join(industry_department.query('院=="電機資訊學院"')['系所'].to_list()) + \
                                '|'.join(industry_department_exception.query('院=="電機資訊學院"')['系所'].to_list())).replace('||','|')
    
    industryProdepartment = '|'.join(industry_department[industry_department[industry]=="v"]['系所'].to_list())
    industryProdepartment_exception = '|'.join(industry_department_exception[industry_department_exception[industry]=="v"]['系所'].to_list())
