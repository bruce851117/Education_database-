...
class BoardFunc():
    ...
    def getProDict():
        ...
        proDict = {'財務':manage_department,
                   '會計':manage_department,
                   '法務':law_department,
                   'IT':ciber_secure_department,
                   '產業專業':f'{industryProdepartment_exception}|{industryProdepartment}'.replace('||','|')
                  }
    ...
    def crtBoardTable(df):  #建立空table存取所有公司各年月的資料
        # 修改 20201201
        df = df[['公司代碼', '公司代碼簡稱', '上市別', 'TSE新產業名', '金融次產業', '銀行分類', '資料源年月', '資料源年']].drop_duplicates()
    ...
    def getAndMergeInfo(df):
        ...
        directorInfo.rename(columns = {'年資董監事' : '年資--董監事'}, inplace = True)
    ...
    def countAndMergeSpecialty(df):
        ...
        for pro in proDict: # 依序整理所有獨董評估項目
            ...
            needDepartment = proDict[pro]
            df2['專業度'] = np.nan
            # 修改 20201201
            # df2['專業度'] = np.where(df2['學院'] == '無法辨識', '無法辨識學院',
                                # np.where(df2['學院'].str.contains(needCollege), '是', '否'))
            df2['專業度'] = np.where(df2['學院'] == '無法辨識', '無法辨識科系',
                                np.where(df2['科系'].str.contains(needDepartment), '是', '否'))
            # if == '無法辨識'
            #    return '無法辨識'
            # else:
            #    if 符合專業標準:
            ...
            # 整合TEJ整理結果&學院專業結果
            # 修改 20201201
            df = df.merge(df2, how='left', on=[col for col in df.columns if col != "專業度"])
            ...
            df_group = df_group[['上市別','公司代碼簡稱','資料源年月','是','否','無法辨識科系']]
            df_group.rename(columns = {'是' : '獨立董事_'+pro+'_符合人數',
                                     '否' : '獨立董事_'+pro+'_不符合人數',
                                     # '無法辨識學院' : '獨立董事_'+pro+'_無法辨識學院人數'
                                     '無法辨識科系' : '獨立董事_'+pro+'_無法辨識科系人數'
                                    }, inplace = True)
            ...
            boardTable['獨立董事_'+pro+'_燈號'] = np.where(boardTable['獨立董事_'+pro+'_符合人數'] > 0, '符合',
                                               np.where(boardTable['獨立董事_'+pro+'_不符合人數'] > 0, '不符合',
                                                   ...
                                                   np.where(boardTable['獨立董事_'+pro+'_無法辨識科系人數'] > 0, '無法辨識科系',
                                                       '沒有揭露')))
            # 計算分數
